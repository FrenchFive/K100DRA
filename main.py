...
def fetch_and_generate_story(project):
    subreddits = [
        'TrueOffMyChest', 'todayilearned', 'TIFU', 'confessions',
        'relationships', 'life', 'decidingtobebetter',
        'offmychest', 'confession', 'FML'
    ]
...
def select_background_video(duration, script_path):
    ...
    files = [f for f in os.listdir(video_folder) if os.path.isfile(os.path.join(video_folder, f)) and f != '.gitkeep']
