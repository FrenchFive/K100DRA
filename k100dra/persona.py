...
class Persona:
    ...
    def story_system_prompt(self, target_seconds: float, max_chars: int,
        ...
        if voiced_chat:
            voice_block = (
                "\nSECOND SPEAKER (REQUIRED: include 1 or 2): a real chat member SPEAKS OUT "
                "LOUD in a separate robotic voice. Write their EXACT words as {chat: their "
                "line} (under 8 words, no tags). This is NOT you saying 'some of you are "
                "saying' — it is the chat member's OWN voiced words, and right after you react "
                "to it. Vary it: sometimes it cuts you off mid-sentence ('okay so he, {chat: "
                "this is so dumb}, no WAIT'), sometimes a quick chime-in. You MUST keep the "
                "{chat: ...} markers exactly in your output.\n")
        if kind == "news":
            premise = (
        ...
        return (
            ...
            f"{self.bio}\n\nFORMAT: {premise}\n\n"
            "Write a first-person, spoken-aloud clip reacting to "
            f"{subject} WITH your chat. MATCH THE LENGTH TO THE SUBSTANCE: if the story is "
            "simple or nothing crazy is actually happening, keep it SHORT, like 12 to 20 "
            "seconds (35 to 55 words), and just stop. Only build up toward "
            f"{int(target_seconds)} seconds (~{words} words, hard max {max_chars} characters) "
            "when it genuinely KEEPS escalating with new wild details. When in doubt, go "
            "SHORT. Never pad to hit a length.\n\n"
